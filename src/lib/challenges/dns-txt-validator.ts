/**
 * RFC 8555 ACME DNS-01 Challenge Validator
 *
 * DNS TXT record validation for ACME challenges according to RFC 8555 Section 8.4.
 * Features:
 * - Authoritative DNS server queries
 * - Zone discovery with NS resolution
 * - Multiple TXT record handling
 * - Base64url validation for challenge tokens
...
/**
 * Result of DNS-01 challenge validation
 */
export interface AcmeDnsValidationResult {
  /** Validation success status */
  ok: boolean;
  /** Matched challenge value from TXT record */
  matched?: string;
  /** All normalized TXT values found */
  allValues: string[];
  /** Validation failure reasons */
  reasons?: string[];
  /** Discovered authoritative zone */
  zone?: string | null;
  /** Authoritative nameserver hostnames */
  nsHosts?: string[];
  /** Resolved nameserver IP addresses */
  nsIPs?: string[];
}

/**
 * Normalize DNS TXT record fragments
 *
 * DNS TXT records can be split into multiple fragments that need concatenation.
 * RFC 1035 specifies that strings in TXT records should be concatenated.
 */
...
/**
 * Validate base64url challenge token format
 *
 * ACME challenge tokens are 32-byte SHA-256 hashes encoded as 43-character
 * base64url strings without padding (RFC 8555 Section 8.1).
 */
export function isValidAcmeChallengeToken(token: string): boolean {
...
export function validateAcmeTxtSet(
  ...
  for (const val of allValues) {
    // shape
    if (!isValidAcmeChallengeToken(val)) {
  ...
  if (!expected) {
    const candidate = allValues.find((v) => isValidAcmeChallengeToken(v) && canDecodeTo32Bytes(v));
    if (candidate) {
...
): Promise<AcmeDnsValidationResult> {
